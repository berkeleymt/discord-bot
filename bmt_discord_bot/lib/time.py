...
class UserFriendlyTime(commands.Converter):
    ...
    async def check_constraints(self, ctx, result, now, remaining):
        if result.dt < now:
            raise commands.BadArgument("This time is in the past.")
        ...
        if self.converter is not None:
            result.arg = await self.converter.convert(ctx, remaining)
        else:
            result.arg = remaining
        return result
    ...
    async def convert(self, ctx, argument):
        ...
        try:
            ...
            if match is not None and match.group(0):
                ...
                result = self.Result(dt=now + relativedelta(**data))
                return await self.check_constraints(ctx, result, now, remaining)
